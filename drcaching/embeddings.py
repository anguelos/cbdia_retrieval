...
class PageRegionEmbeddings(object):
    @classmethod
    def load_external(cls, region_filename, embeddings):
        path = pathlib.Path(region_filename)
        book_id, page_num, md5, region_type = path.stem.split("_")
        book_id = int(book_id)
        page_num = int(page_num)
        book_root = path.parent
        return cls(embeddings, book_id=book_id, page_num=page_num, book_root=book_root, md5=md5,
    ...
    def load(cls, filename, page=None, regions=None):
        ...
        if pathlib.Path(filename).suffix == ".json":
            embeddings = json_load_np(filename)
        elif pathlib.Path(filename).suffix == ".pickle":
            embeddings = pickle.load(open(filename))
        else:
        ...
        page_num = int(page_num)
        book_root = path.parent
        return cls(embeddings=embeddings, book_id=book_id, page_num=page_num, book_root=book_root, md5=md5,
                   region_type=region_type, page=page, regions=regions)

    def __init__(self, embeddings, book_id, page_num, book_root, md5, region_type, page=None, regions=None):
        self._page = page
    ...
    @property
    def id(self):
        return "{}_{}_{}_{}_embedding".format(self.book_id, self.page_num, self.md5, self.region_type)
    ...
    def save(self):
        if self.binary_io:
            pickle.dump(open(self.get_binary_filename(), "w"), self.embeddings)
        else:
            json_save_np(self.embeddings, self.get_text_filename())
